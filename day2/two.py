...
input_lines = [line for line in input.splitlines()]
...
for line in input_lines:
    ...
    upper = int(parts[1].split(' ')[0].strip())
    character = parts[1].split(' ')[1][0]
    password = parts[1].split(': ')[1]
    count = password.count(character)
    if max(upper, lower) - 1 >= len(password):
        continue
    if (password[lower - 1] == character or password[upper - 1] == character) and not (password[lower - 1] == character and password[upper - 1] == character):
        print(line)
        sum += 1
