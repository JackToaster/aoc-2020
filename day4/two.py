...
for passport in passports:
    if all(" " + field + ":" in passport for field in fields):
        try:
            for field in fields:
                pos = passport.find(" " + field + ":") + len(" " + field + ":")
                end_pos = passport.find(" ", pos)
                if end_pos == -1:
                    field_val = passport[pos:]
                else:
                    field_val = passport[pos:end_pos]

                if field == 'byr':
                    if not 1920 <= int(field_val) <= 2002:
                        raise ValueError('byr')
                elif field == 'iyr':
                    if not 2010 <= int(field_val) <= 2020:
                        raise ValueError('iyr')
                elif field == 'eyr':
                    if not 2020 <= int(field_val) <= 2030:
                        print(field_val)
                        raise ValueError('eyr')
                elif field == 'hgt':
                    if pos := field_val.find("cm") != -1:
                        pos = field_val.find("cm")
                        if not 150 <= int(field_val[0:pos]) <= 193:
                            raise ValueError('hgt-cm')
                    elif pos := field_val.find("in") != -1:

                        pos = field_val.find("in")
                        if not 59 <= int(field_val[0:pos]) <= 76:
                            raise ValueError('hgt-in')
                    else:
                        raise ValueError('hgt')
                elif field == 'hcl':
                    if field_val[0] != '#':
                        raise ValueError('hcl-#')
                    if any(chara not in "1234568790abcdef" for chara in field_val[1:7]):
                        raise ValueError('hcl')
                elif field == 'ecl':
                    if field_val not in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']:
                        print(field_val)
                        raise ValueError('ecl')
                elif field == 'pid':
                    if len(field_val) != 9 or any(chara not in '0123456879' for chara in field_val):
